...
describe('Can use admin features', () => {
  ...
  it('should be able able to delete user', () => {
    ...
    cy.findAllByText(/15th medical group/i)
      .eq(4)
      .click();
  ...
  it.only('should be able to merge user accounts', () => {
    ...
    cy.findByTestId('winner-account').type(emailToChange);
    cy.select(emailToChange);

    cy.findByTestId('loser-account').type(newEmail).select(newEmail);
